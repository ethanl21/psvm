...
ShowdownService::ShowdownService() : pimpl(new impl) {
    ...
    qjs::js_std_eval_binary(this->pimpl->ctx_, psvmjs::qjsc_globalize, psvmjs::qjsc_globalize_size, 0);

    // Initialize ShowdownService instance
    auto create_sim_eval = "globalThis.ShowdownServiceInstance = new psvm.ShowdownService();";
    auto r = qjs::JS_Eval(this->pimpl->ctx_, create_sim_eval, strlen(create_sim_eval), "", JS_EVAL_TYPE_GLOBAL);
