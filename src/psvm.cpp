...
namespace psvm {
    ...
    static JSValue ResponseCallback(JSContext *ctx, JSValueConst, int argc, JSValueConst *argv) {
    ...
    ShowdownService::ShowdownService() : pimpl_(new Impl) {
        ...
        JS_SetPropertyStr(pimpl_->ctx, global, "ResponseCallback",
        ...
        JSValue ctor = JS_GetPropertyStr(pimpl_->ctx, global_obj, "showdownService");
        pimpl_->js_showdownService = JS_CallConstructor(pimpl_->ctx, ctor, 0, nullptr);
        pimpl_->js_writeToBattle = JS_GetPropertyStr(pimpl_->ctx, pimpl_->js_showdownService, "writeToBattle");
    ...
    void ShowdownService::setCallback(std::function<void(const std::string &, const std::string &)> cb) {
        pimpl_->callback_data->user_callback = std::move(cb);
    }

    void ShowdownService::writeToBattle(const std::string &id, const std::string &message) {
        JSContext *ctx = pimpl_->ctx;
        JSValue args[2] = {JS_NewString(ctx, id.c_str()), JS_NewString(ctx, message.c_str())};

        JSValue ret = JS_Call(ctx, pimpl_->js_writeToBattle, pimpl_->js_showdownService, 2, args);
    ...
    void ShowdownService::startBattle(const std::string &id) {
        JSContext *ctx = pimpl_->ctx;
        JSValue func = JS_GetPropertyStr(ctx, pimpl_->js_showdownService, "startBattle");
    ...
    void ShowdownService::killBattle(const std::string &id) {
        JSContext *ctx = pimpl_->ctx;
        JSValue func = JS_GetPropertyStr(ctx, pimpl_->js_showdownService, "killBattle");
        if (!JS_IsUndefined(func)) {
            JSValue arg = JS_NewString(ctx, id.c_str());
            JSValue ret = JS_Call(ctx, func, pimpl_->js_showdownService, 1, &arg);
            JS_FreeValue(ctx, arg);
    ...
    void ShowdownService::killAllBattles() {
        JSContext *ctx = pimpl_->ctx;
        JSValue func = JS_GetPropertyStr(ctx, pimpl_->js_showdownService, "killAllBattles");
        if (!JS_IsUndefined(func)) {
            JSValue ret = JS_Call(ctx, func, pimpl_->js_showdownService, 0, nullptr);
            JS_FreeValue(ctx, ret);
            js_std_loop(ctx);
        }
        JS_FreeValue(ctx, func);
